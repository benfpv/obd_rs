...
from ._helpers import _content_rect
from ._text import STYLES, draw_text, fit_text_to
...
class _AlertsPanel:
    ...
    def draw(self, frame: np.ndarray, rect: tuple, theme: dict) -> None:
        ...
        if not self._alerts:
            # One-off scale tuned for empty state; keep inline rather than
            # bloating the style registry.
        ...
        for i, alert in enumerate(self._alerts[:max_lines]):
            ...
            txt = fit_text_to(f"{alert.title} | {alert.detail}", max(24, w - 10), STYLES.caption)
            draw_text(frame, txt, (x + 6, ay + 14), theme["text_primary"], STYLES.caption)
