...
class cvplt:
    ...
    def draw_plot(data, renderArray=None, plotBeginXY=None, plotEndXY=None,
        ...
        Args:
            ...
            yRange (tuple[float, float]|None): Optional fixed (min, max) bounds
                for plotting. When provided, axis labels remain stable instead of
                auto-scaling each frame.
        ...
        dataToPlot, dataLenToPlot, dataRange = cvplt._plotArray_process_data(data, dataLen, plotArraySizeActual, yRange)
    ...
    @staticmethod
    def _plotArray_process_data(data, dataLen, plotArraySizeActual, yRange=None):
        ...
        if yRange is not None:
            try:
                y_min = float(yRange[0])
                y_max = float(yRange[1])
                if np.isfinite(y_min) and np.isfinite(y_max) and y_max > y_min:
                    dataRange = np.array([y_min, y_max], dtype="float32")
                else:
        ...
        # Fixed y-range should map absolute values to absolute plot positions.
        if yRange is not None:
            dataToPlot = (data - dataRange[0]) * dataMultiplier
    ...
    def _plotArray_draw_text(plotTitle, plotArray, plotArraySize, dataRange, plotOutlineColour):
        # Classic compact style with rounded values for readability.
    ...
    def _plotArray_draw_text_coords(plotTitle, plotArray, plotArraySize, dataRange, plotOutlineColour):
        ...
        fontPositionTuningBot = int(.007 * plotArraySize[1])
        fontSizeTuning = (.001 * plotArraySize[1])
        textMax = "({},{})  |  {}".format(round(float(dataRange[0][0]), 1), round(float(dataRange[0][1]), 1), plotTitle)
        plotArray = cv2.putText(plotArray, textMax, [3, 11 + fontPositionTuningTop], font, .4 + fontSizeTuning, plotOutlineColour, 1)
        textMin = "({},{})".format(round(float(dataRange[1][0]), 1), round(float(dataRange[1][1]), 1))
        plotArray = cv2.putText(plotArray, textMin, [3, plotArraySize[1]-5 - fontPositionTuningBot], font, .4 + fontSizeTuning, plotOutlineColour, 1)
        return plotArray
